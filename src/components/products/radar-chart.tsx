export function RadarChart({ 
    ...
    const scores = [data.usability, data.features, data.price, data.community, data.overall];
    ...
    return (
      <div className="relative flex flex-col items-center">
        <svg width={size} height={size} className="overflow-visible">
            {/* Background Grid */}
            <polygon points={bgPoints} fill="#F8FAFC" stroke="#172554" strokeWidth="2" />
            <polygon points={midPoints} fill="none" stroke="#172554" strokeWidth="1" strokeDasharray="4 4" opacity="0.3" />
            
            {/* Data Polygon */}
            <polygon points={polyPoints} fill="#FDE047" fillOpacity="0.6" stroke="#172554" strokeWidth="3" />
            ...
            {angles.map((angle, i) => {
                ...
                const labelX = center + labelR * Math.cos(angle - Math.PI / 2);
                const labelY = center + labelR * Math.sin(angle - Math.PI / 2);

                return (
                    <g key={i}>
                        <line x1={center} y1={center} x2={x} y2={y} stroke="#172554" strokeWidth="1" opacity="0.2" />
                        <text 
                            x={labelX} 
                            y={labelY} 
                            textAnchor="middle" 
                            dominantBaseline="middle" 
                            className="text-[10px] font-bold fill-[#172554] uppercase tracking-wide"
                        >
                            {labels[i]}
                        </text>
                        {/* Score Value */}
                         <text 
                            ...
                            textAnchor="middle" 
                            dominantBaseline="middle" 
                            className="text-[10px] font-bold fill-[#1D4ED8]"
                        >
                            {scores[i]}
                        </text>
                    </g>
