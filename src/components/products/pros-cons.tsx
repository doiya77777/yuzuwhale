...
export function ProsCons({ pros, cons }: { pros: string[]; cons: string[] }) {
    return (
        <div className="grid gap-6 md:grid-cols-2">
            {/* Pros */}
            <div className="rounded-2xl soft-card bg-[var(--yuzu-mint)] p-5">
                ...
                <ul className="space-y-3">
                    {pros.map((pro, i) => (
                        <li
                            key={i}
                            className="flex items-start gap-2 text-sm font-semibold text-[#14532d]"
                        >
            ...
            {/* Cons */}
            <div className="rounded-2xl soft-card bg-[var(--yuzu-blush)] p-5">
                ...
                <ul className="space-y-3">
                    {cons.map((con, i) => (
                        <li
                            key={i}
                            className="flex items-start gap-2 text-sm font-semibold text-[#7f1d1d]"
                        >
